...
import CurieLifeError, { networkError } from './curieLifeError'
import { CurieLifeStatusCodes } from './statusCodes'
...
export default class ApiRequest {
  ...
  performErroCheck?: (arg0: CurieLifeBaseResponse) => Promise<any>

  // Used when only error check needed is HTTP status code check. In such case,
  // Just resolve the promise using API response.
  successReturn = (arg0: CurieLifeBaseResponse): Promise<any> =>
    Promise.resolve(arg0)

  constructor(
    url: string,
    customErrorCheck?: (arg0: CurieLifeBaseResponse) => any | undefined
  ) {
    this.baseUrl = url
    ...
    return fetch(
      ...
      .catch((error: CurieLifeError) => {
        if (error.message === 'Failed to fetch') {
          const err = networkError(CurieLifeStatusCodes.NETWORK_FAILED)
